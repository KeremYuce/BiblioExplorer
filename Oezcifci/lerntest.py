...
def perform_ocr(image, output_file):
    ...
    # NEU: Hier sammeln wir ALLE erkannten Textboxen
    text_data = []
    ...
    for i in range(num_boxes_black):
        ...
        if text.strip():  # Text nicht leer
            ...
            # Nur Boxen innerhalb des gezeichneten Rechtecks
            if (x + w >= x1 and x <= x2) and (y + h >= y1 and y <= y2):
                center_x, center_y = calculate_center(x, y, w, h)

                # Rechteck & Text ins Bild einzeichnen (wie vorher)
                cv2.rectangle(image, (x, y), (x + w, y + h), (0, 255, 0), 2)
                cv2.putText(image, f"({x}, {y})", (x, y - 10),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
                cv2.putText(image, f"({int(center_x)}, {int(center_y)})",
                            (int(center_x), int(center_y) - 10),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)

                # NEU: Daten in text_data speichern
                text_data.append({
                    "x": x,
                    "y": y,
                    "w": w,
    ...
    for i in range(num_boxes_white):
        ...
        if text.strip():  # Text nicht leer
            ...
            if (x + w >= x1 and x <= x2) and (y + h >= y1 and y <= y2):
                ...
                # Rechteck & Text ins Bild einzeichnen (rot für weißen Text)
                ...
                with open(output_file, 'a') as file:
                    file.write(f"----------------------------------------\n")
                    file.write(f"Erkannter Text: '{text}'\n")
                    file.write(f"Koordinaten: ({x}, {y}) - ({x + w}, {x + w})\n")
                    file.write(f"Mittelpunkt: ({int(center_x)}, {int(center_y)})\n")
                    file.write(f"----------------------------------------\n\n")

    # NEU: KMeans-Clustering auf den gesammelten Text-Boxen
    if len(text_data) > 0:
        clustered_image = perform_clustering_with_buch(text_data, image.copy(), output_file)
        cv2.imshow("OCR + Clustering", clustered_image)
    else:
        # Falls nichts erkannt wurde
        cv2.imshow("OCR + Clustering", image)
